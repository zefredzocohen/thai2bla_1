...
(function ($) {
	...
	var cache	= {
			...
			sliceH			: 110,  // the default slice's height	
		...
		aux		= {
				...
				return $.Deferred(
					function( dfd ) {
						...
						if( expanded ) {
							...
							itemHeight = settings.itemHeight; //cache.sliceH;
							othersHeight = settings.itemHeight; //cache.sliceH;
							...
							if( aux.canSlideUp( $slices, settings ) )
								$navPrev.removeClass('disable')//.fadeIn();
							else
								$navPrev.addClass('disable')//.fadeOut();
								
							if( aux.canSlideDown( $slices, settings ) )
							    $navNext.removeClass('disable')//.fadeIn();
							else
							    $navNext.addClass('disable')//.fadeOut();
						}
						// if it's closed..
						else {
							...
							itemHeight = settings.itemHeight;//settings.expandedHeight;
							// the height the other slices will have
							othersHeight = settings.itemHeight;//settings.expandedHeight; //Math.ceil((settings.accordionH - settings.expandedHeight) / (settings.visibleSlices - 1));
						...
						$others.each(function(i){
							...
							$other.stop()
								  .animate( {
										...
										opacity	: 1//( expanded ) ? 1 : settings.animOpacity
			...
			slide: function (dir, $slices, $navNext, $navPrev, settings) {
			    $slices = $slices.parent().find('.va-slice:not(.hidden)');
				...
				$slices.each(function (i) {
					...
					if (!$slice.hasClass('hidden') && $slices.length > 1) {
					    ...
					    if (!settings.savePositions || cache.idx_expanded === -1) {
					        ...
					        if (i === $nextSlice.index()) {
					            ...
					            itemHeight = settings.itemHeight; //settings.expandedHeight;
					            othersHeight = settings.itemHeight; //(settings.accordionH - settings.expandedHeight) / (settings.visibleSlices - 1);
					            ...
					            $slice.stop()
                                      .animate({
                                          height: othersHeight + 'px',
                                          opacity: 1, //settings.animOpacity,
				...
				if( this.length ) {
					...
					return this.each(function () {
						...
						var $el 			= $(this),
							...
							$slices			= $el.find('div.va-slice:not(.hidden)'),
							// the navigation buttons
							$navNext		= $el.find('span.va-nav-next'),
							$navPrev		= $el.find('span.va-nav-prev');
						
						// each slice's height
						cache.sliceH        = settings.itemHeight;//Math.ceil( settings.accordionH / settings.visibleSlices );
						...
						if (settings.expandedHeight > settings.accordionH)
						    ...
						    settings.expandedHeight = settings.itemHeight; //cache.sliceH + 50; // give it a minimum
						...
						if( settings.visibleSlices < cache.totalSlices  )
						    $navNext.removeClass('disable');//.show();
						...
						$slices.each(function(i){
						    var $slice = $(this);
						    if (!$slice.hasClass('hidden')) {
						        $slice.css({
						            top: iShowItem * cache.sliceH + 'px',
						            height: cache.sliceH + 'px'
						        }).data('position', (iShowItem + 1));
						        iShowItem++;
						    }
						})
						.children('.va-title')
