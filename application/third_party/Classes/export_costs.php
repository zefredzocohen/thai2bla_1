...
$this->excel->getActiveSheet()->getRowDimension(5)->setRowHeight(12.75);
 ...
 if($cost_type == 'thu' || $cost_type == 'all'){
   if($cost_type == 'thu'){  
    $this->excel->setActiveSheetIndex(0)->mergeCells('G8:H8');
   }
 $this->excel->getActiveSheet()->setCellValue('G8', "THU");
 }if($cost_type == 'chi' || $cost_type == 'all'){
     if($cost_type == 'chi'){  
    $this->excel->setActiveSheetIndex(0)->mergeCells('G8:H8');
    $this->excel->getActiveSheet()->setCellValue('G8', "CHI");
   }
...
foreach ($rows as $key=>$r) {
	  ...
	  $this->excel->getActiveSheet()->setCellValue('F' . $k, $r[5]);
          if($cost_type == 'thu' || $cost_type == 'all'){
            if($cost_type == 'thu'){
             $this->excel->setActiveSheetIndex(0)->mergeCells('G' . ($k).':H'.($k));
            }
	  $this->excel->getActiveSheet()->setCellValue('G' . $k, $r[6]);
          }if($cost_type == 'chi' || $cost_type == 'all'){
               if($cost_type == 'chi'){
             $this->excel->setActiveSheetIndex(0)->mergeCells('G' . ($k).':H'.($k));
             $this->excel->getActiveSheet()->setCellValue('G' . $k, $r[7]);
            }
...
/* end border cot */
  ...
  if($cost_type == 'thu' || $cost_type == 'all'){
      if($cost_type == 'thu'){
       $this->excel->setActiveSheetIndex(0)->mergeCells('G' . ($i).':H'.($i));
      }
  $this->excel->getActiveSheet()->setCellValue('G' . ($i), to_currency_unVND_nomar($tien_thu_tong));
  }if($cost_type == 'chi' || $cost_type == 'all'){
       if($cost_type == 'chi'){
       $this->excel->setActiveSheetIndex(0)->mergeCells('G' . ($i).':H'.($i));
       $this->excel->getActiveSheet()->setCellValue('G' . ($i), to_currency_unVND_nomar($tien_chi_tong));
      }
