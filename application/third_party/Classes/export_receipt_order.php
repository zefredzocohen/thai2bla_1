...
;
$address1 = $cust_info->address_1;
$info_sale_item = $this->Sale->get_sale_item_order();
if ($info_sale->suspended == 1) {
    $order_name = "HÓA ĐƠN GHI NỢ";
} elseif ($info_sale->liability == 1) {
    $order_name = 'HÓA ĐƠN ĐẶT HÀNG';
} elseif ($info_sale->materials == 1) {
    $order_name = 'HÓA ĐƠN BÁO GIÁ';
} elseif ($info_sale->later_cost_price < 0) {
    $order_name = "HÓA ĐƠN TRẢ HÀNG";
} else {
    $order_name = "HÓA ĐƠN BÁN HÀNG";
}

$this->excel->setActiveSheetIndex(0)->mergeCells('D2:G2');
$this->excel->getActiveSheet()->setCellValue('D2', $order_name);
...
$this->excel->getActiveSheet()->setCellValue('I4', $sale_id);
...
$objRed12 = $objRichText12->createTextRun('Nhân viên bán hàng');
//$objRed->getFont()->setColor("FFFF0000");
$objRed12->getFont()->setBold(true);
$objGreen12 = $objRichText12->createTextRun(' (Sale customer):');
...
$employee_sale = $this->Employee->get_info($info_sale->employee_id)->first_name . ' ' . $this->Employee->get_info($info_sale->employee_id)->last_name;
$this->excel->getActiveSheet()->setCellValue('D11', $employee_sale);
...
$this->excel->getActiveSheet()->setCellValue('D14', ' Ông/bà (Mr/Ms): ' . $cus_name, MB_CASE_UPPER, "UTF-8");
...
$this->excel->getActiveSheet()->setCellValue('D17', $address1);
...
$this->excel->getActiveSheet()->setCellValue('E22', "ĐV tính");
$this->excel->getActiveSheet()->setCellValue('F22', "Số lượng");
$this->excel->getActiveSheet()->setCellValue('G22', "Đơn giá");
$this->excel->getActiveSheet()->setCellValue('H22', "Chiết khấu");
...
$this->excel->getActiveSheet()->setCellValue('E23', "(Units)");
...
$this->excel->getActiveSheet()->setCellValue('F23', "(Quantity)");
...
$this->excel->getActiveSheet()->setCellValue('G23', "(Unit price)");
...
$this->excel->getActiveSheet()->setCellValue('H23', "(Discount)");
...
//DEN DAY
$this->excel->getActiveSheet()->getStyle('E22:I22')->getFont()->setBold(true);
...
$this->excel->setActiveSheetIndex(0)->mergeCells('C24' . ':D24');
...
$this->excel->getActiveSheet()->setCellValue('E24', "C");
$this->excel->getActiveSheet()->setCellValue('F24', "1");
$this->excel->getActiveSheet()->setCellValue('G24', "2");
$this->excel->getActiveSheet()->setCellValue('H24', "3");
$this->excel->getActiveSheet()->setCellValue('I24', "4 = (1x2)-(1x2x3)");
...
$this->excel->getActiveSheet()->getStyle('E24:I24')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
/* */
        ...
        if ($sale_id == $val['sale_id']):
            foreach ($get_taxes_item as $get_value):
                if ($val['item_id'] == $get_value['item_id']):
                    ...
                    $total_tax = ($item_price * $val['quantity_purchased'] - $item_price * $val['quantity_purchased'] * $val['discount_percent'] / 100) * $get_value['taxes'] / 100;
                    if ($val['unit_item'] == $get_value['unit_from']) {
...
//$this->excel->setActiveSheetIndex(0)->mergeCells('C'.$k.':C'.($k+1));	
                    $this->excel->getActiveSheet()->setCellValue('C' . ($k), $this->Item->get_info($val['item_id'])->name);
...
//$this->excel->setActiveSheetIndex(0)->mergeCells('D'.($k+1).':E'.($k+1));
                    $this->excel->getActiveSheet()->setCellValue('D' . ($k), $val['item_id']);
...
//$this->excel->getActiveSheet()->setCellValue('D'.($k+1),$this->Item->get_info($item['item_id'])->images);
                    ...
                    $anh = $this->Item->get_info($val['item_id'])->images;
                    if ($anh != "") {
                        $imagePath = 'items/' . $anh;
                        $objDrawing = new PHPExcel_Worksheet_Drawing();
                        $objDrawing->setPath($imagePath);
...
$this->excel->getActiveSheet()->getStyle('D'.($k+1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    ...
                    $this->excel->setActiveSheetIndex(0)->mergeCells('E' . $k . ':E' . ($k + 1));
                    $this->excel->getActiveSheet()->setCellValue('E' . $k, $this->Unit->item_unit($this->Item->get_info($item_unit)->unit)->name);
                    ...
                    $this->excel->getActiveSheet()->setCellValue('F' . $k, $val['quantity_purchased']);
                    ...
                    $this->excel->getActiveSheet()->setCellValue('G' . $k, to_currency_unVND_nomar($item_price));
                    ...
                    $this->excel->getActiveSheet()->setCellValue('H' . $k, ($val['discount_percent']));
                    $this->excel->getActiveSheet()->getStyle('H' . $k)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

                    $this->excel->getActiveSheet()->setCellValue('I' . $k, number_format($val['quantity_purchased'] * ($item_price - ($item_price * $val['discount_percent'] / 100))));
...
$objRed = $objRichText->createTextRun('Tổng giá trị đơn hàng');
...
$this->excel->getActiveSheet()->setCellValue('I' . $i, number_format($total_money));
...
$this->excel->getActiveSheet()->setCellValue('D' . ($i + 1), "Tổng thuế: ");
...
$this->excel->getActiveSheet()->setCellValue('I' . ($i + 1), number_format($tt));
...
if ($info_sale->suspended == 1 || $info_sale->liability == 1 || $info_sale->materials == 1) {
    ...
    $this->excel->setActiveSheetIndex(0)->mergeCells('B' . ($i + 4) . ':G' . ($i + 4));
    $objRichText1 = new PHPExcel_RichText();
    $objRed1 = $objRichText1->createTextRun('Còn nợ :');
//$objRed->getFont()->setColor("FFFF0000");
    ...
    $this->excel->getActiveSheet()->getCell('B' . ($i + 4))->setValue($objRichText1);
    $this->excel->getActiveSheet()->getStyle('B' . ($i + 4) . ':I' . ($i + 4))->getBorders()->getTOP()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    $this->excel->getActiveSheet()->getStyle('B' . ($i + 4))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $this->excel->getActiveSheet()->getStyle('B' . ($i + 4))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);



    $this->excel->getActiveSheet()->setCellValue('I' . ($i + 4), number_format(($total_money + $total_tax) - $payment - $discount_money));

    $this->excel->getActiveSheet()->getStyle('I' . ($i + 4))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    $this->excel->getActiveSheet()->getStyle('I' . ($i + 4))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $this->excel->getActiveSheet()->getStyle('I' . ($i + 4))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
} else {
    $this->excel->setActiveSheetIndex(0)->mergeCells('B' . ($i + 4) . ':G' . ($i + 4));
    $objRichText1 = new PHPExcel_RichText();
    $objRed1 = $objRichText1->createTextRun('Tiền trả lại khách :');
//$objRed->getFont()->setColor("FFFF0000");
    $objRed1->getFont()->setBold(true);
    $objGreen1 = $objRichText1->createTextRun(' (Total):');
    $objRichText1->createText(' ');
    ...
    $this->excel->getActiveSheet()->setCellValue('I' . ($i + 4), number_format(($total_money + $total_tax) - $payment));
...
$this->excel->getActiveSheet()->getStyle('B' . ($i + 2) . ':G' . ($i + 2))->getBorders()->getRIGHT()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
// $discount = $this->Sale->get_discount_money();
//                    foreach ($discount as $discount_value) {
//                        if ($sale_id == $discount_value['sale_id']) {
//                            $discount_money = $discount_value['discount_money'];
...
$this->excel->getActiveSheet()->setCellValue('B' . ($i + 2), "Chiết khấu tiền mặt : ");
...
$this->excel->getActiveSheet()->getStyle('I' . ($i + 2))->getFont()->setSize(11);


$this->excel->getActiveSheet()->setCellValue('I' . ($i + 2), number_format($discount_money));
...
$this->excel->getActiveSheet()->setCellValue('B' . ($i + 3), "Đã thanh toán : ");
...
$this->excel->getActiveSheet()->setCellValue('I' . ($i + 3), number_format($payment));
...
///////////////////
$this->excel->setActiveSheetIndex(0)->mergeCells('B' . ($i + 5) . ':F' . ($i + 5));
$this->excel->getActiveSheet()->getStyle('B' . ($i + 5))->getFont()->setSize(9);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 5))->getFont()->setBold(true);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 5) . ':I' . ($i + 5))->getBorders()->getTOP()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 5) . ':G' . ($i + 5))->getBorders()->getRIGHT()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
$this->excel->getActiveSheet()->setCellValue('B' . ($i + 5), "Số tiền bằng chữ : ");

$this->excel->getActiveSheet()->getStyle('B' . ($i + 5))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$this->excel->getActiveSheet()->getStyle('I' . ($i + 5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
$this->excel->getActiveSheet()->getStyle('I' . ($i + 5))->getFont()->setSize(11);

$this->load->model("Cost");
$this->excel->getActiveSheet()->setCellValue('I' . ($i + 5), $this->Cost->get_string_number($payment));

$this->excel->getActiveSheet()->getStyle('I' . ($i + 5))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
////////////////////
...
$objRed3->getFont()->setSize(10);
$objRed3->getFont()->setName('Times New Roman');
$objGreen3->getFont()->setSize(10);
$objGreen3->getFont()->setItalic(true);
$objGreen3->getFont()->setName('Times New Roman');
$this->excel->getActiveSheet()->getCell('B' . ($i + 6))->setValue($objRichText3);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 6))->getAlignment()->setIndent(1);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 6))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
//

$this->excel->setActiveSheetIndex(0)->mergeCells('D' . ($i + 6) . ':G' . ($i + 6));
...
$objRed90->getFont()->setSize(10);
$objRed90->getFont()->setName('Times New Roman');
$objGreen90->getFont()->setSize(10);
$objGreen90->getFont()->setItalic(true);
$objGreen90->getFont()->setName('Times New Roman');
$this->excel->getActiveSheet()->getCell('D' . ($i + 6))->setValue($objRichText90);
$this->excel->getActiveSheet()->getStyle('D' . ($i + 6))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$this->excel->setActiveSheetIndex(0)->mergeCells('H' . ($i + 6) . ':I' . ($i + 6));
...
$objRed4->getFont()->setSize(10);
$objRed4->getFont()->setName('Times New Roman');
$objGreen4->getFont()->setSize(10);
$objGreen4->getFont()->setItalic(true);
$objGreen4->getFont()->setName('Times New Roman');
$this->excel->getActiveSheet()->getCell('H' . ($i + 6))->setValue($objRichText4);
$this->excel->getActiveSheet()->getStyle('H' . ($i + 6))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$this->excel->setActiveSheetIndex(0)->mergeCells('B' . ($i + 7) . ':I' . ($i + 7));
$this->excel->getActiveSheet()->setCellValue('B' . ($i + 7), '  (Ký, ghi rõ họ tên - Sign,Fullname)                                    (Ký, ghi rõ họ tên - Sign,Fullname)           (Đóng dấu ghi rõ họ tên - Sign, Stamp,Fullname)');
$this->excel->getActiveSheet()->getStyle('B' . ($i + 7))->getFont()->setItalic(true);
$this->excel->getActiveSheet()->getStyle('B' . ($i + 7))->getFont()->setSize(8);

$this->excel->setActiveSheetIndex(0)->mergeCells('B' . ($i + 9) . ':I' . ($i + 9));
//$this->excel->getActiveSheet()->getHeaderFooter()->setOddFooter('&L&B' .);
$this->excel->getActiveSheet()->setCellValue('B' . ($i + 9), "Ghi chú: Quý khách hàng xin vui lòng kiểm tra kỹ đơn hàng và ký nhận đầy đủ");
$this->excel->getActiveSheet()->getStyle('B' . ($i + 9))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$this->excel->getActiveSheet()->getStyle('B' . ($i + 9))->applyFromArray($styleArray);


$this->excel->getActiveSheet()->getStyle('A' . ($i + 13) . ':I' . ($i + 14))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$this->excel->getActiveSheet()->getStyle('A' . ($i + 13) . ':I' . ($i + 14))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
/* */
/* */
...
$this->excel->getActiveSheet()->getStyle('D22:D24')->getBorders()->getRIGHT()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
$this->excel->getActiveSheet()->getColumnDimension('E')->setWidth(8.15);
$this->excel->getActiveSheet()->getColumnDimension('F')->setWidth(8.15);
$this->excel->getActiveSheet()->getColumnDimension('G')->setWidth(12.5);
$this->excel->getActiveSheet()->getColumnDimension('H')->setWidth(8.9);
