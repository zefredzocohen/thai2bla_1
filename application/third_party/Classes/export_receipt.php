...
//	die('ssss');
	...
	$this->excel->getActiveSheet()->setCellValue('H'.$k,($item['discount']));
	...
	$this->excel->getActiveSheet()->setCellValue('I'.$k,to_currency_unVND_nomar($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100));
	    ...
	    $this->excel->getActiveSheet()->getStyle('I'.$k)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);	$this->excel->getActiveSheet()->getStyle('B'.$k.':I'.$k)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
...
$this->excel->getActiveSheet()->getCell('B'.($i+3))->setValue($objRichText1);
...
$this->excel->getActiveSheet()->getStyle('B'.($i+3))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
 ...
 foreach($data['payments'] as $payment_id=>$payment) 
  {
  	$ck += $payment['discount_money'];
  }
 
	$this->excel->getActiveSheet()->setCellValue('I'.($i+3),to_currency_unVND_nomar($data['total']-$ck));
...
$this->excel->getActiveSheet()->getStyle('I'.($i+2))->getFont()->setSize(11);
 ...
 foreach($data['payments'] as $payment_id=>$payment)  
 {
	$this->excel->getActiveSheet()->setCellValue('I'.($i+2),to_currency_unVND_nomar($payment['discount_money']));
 }
$this->excel->getActiveSheet()->getStyle('I'.($i+2))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
//if($payments_cost > 0){
...
$this->excel->setActiveSheetIndex(0)->mergeCells('D'.($i+5).':G'.($i+5));
...
$this->excel->setActiveSheetIndex(0)->mergeCells('H'.($i+5).':I'.($i+5));
...
$this->excel->getActiveSheet()->getCell('H'.($i+5))->setValue($objRichText4);
$this->excel->getActiveSheet()->getStyle('H'.($i+5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$this->excel->getActiveSheet()->setCellValue('B'.($i+8),"Ghi chú: Quý khách hàng xin vui lòng kiểm tra kỹ đơn hàng và ký nhận đầy đủ");
$this->excel->getActiveSheet()->getStyle('B'.($i+8))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$this->excel->getActiveSheet()->getStyle('B'.($i+8))->applyFromArray($styleArray);
