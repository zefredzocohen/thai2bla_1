...
//name the worksheet
$this->excel->getActiveSheet()->setTitle('Báo cáo thu theo ngày');
...
$this->excel->getActiveSheet()->setCellValue('A2',$this->config->item('address'));
	...
	$this->excel->getActiveSheet()->setCellValue('A4', "TỔNG CÁC KHOẢN THU THEO NGÀY");
...
$this->excel->getActiveSheet()->setCellValue('H6', "Tiền thu");
...
$stt=1;
$subtotal=0;
foreach($thu_date as $query){
			
			$this->excel->getActiveSheet()->setCellValue('A'.$k, $stt );
			$this->excel->getActiveSheet()->setCellValue('B'.$k, date('d-m-Y H:i:s', strtotime($query['cost_date_ct'])) );
			...
			if ($query['tien_thu']>0) {
		$this->excel->getActiveSheet()->setCellValue('H'.$k, to_currency_unVND_nomar($query['tien_thu']));
			} else {
...
$this->excel->getActiveSheet()->getStyle('E' . ($i-2))->getFont()->setBold(true);
 ...
 $this->excel->getActiveSheet()->setCellValue('H' . ($i-2),to_currency_unVND_nomar($subtotal));
...
$filename = 'baocaothu_theongay.xlsx'; //save our workbook as this file name
//$filename = 'Book1.xlsx';
