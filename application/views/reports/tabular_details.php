...
<div id="content_area" style="height: auto; float: left;"><script
...
<table id="title_bar">
	<tr>
		...
		<td id="title"><?php echo lang('reports_report_customer'); ?></td>
		<td><a
			style="font-size: 18px; text-decoration: underline; color: #FFF"
			href="<?php echo base_url();?>reports/specific_customer">Trở
                        lại</a></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
...
<div style="margin-top: 10px;">
    <?php if($person_id==0){?>
     <P>KHÁCH HÀNG : <font color="red">Khách lẻ</font></P>
    <?php }else{?>
      <P>KHÁCH HÀNG : <font color="red"><?php echo $title;?></font></P>
    <?php }?>
</div>
<br>
<table id="contents">
	<tr>
		...
		<table class="tablesorter report" id="sortable_table">
			...
			$total_real = 0;
			?>
			<?php foreach ($data_all_sale as $key => $val) { ?>
			<?php foreach ($val as $key1 => $val1) { ?>
				<tr>
					...
					<td><?= $val1['first_name1'] . ' ' . $val1['last_name1'] ?></td>
					<?php foreach ($info_total_sale as $key2 => $val2) {
						if ($key2 == $key) {
							$total_price = 0;
							$total_item = 0;
							$later_cost_price = 0;
...
//                                                                                        $total_price += $price*$val5['quantity_purchased']-$price*$val5['quantity_purchased']*$val5['discount_percent']/100;
										}
									}
								}
							}
					...
					<table class="innertable">
						...
						foreach ($detail_sale as $key3 => $val3) {
							...
							<tr>
								...
								$this->Unit->get_info($val5['unit_item'] )->name ?></td>
		...
		<div class="summary_row"><strong>Tổng chiết khấu: </strong><?= to_currency($total_discount) ?>
		<div class="summary_row"><strong>Tổng thực thu: </strong><?= to_currency($total_real) ?>
		</div>
