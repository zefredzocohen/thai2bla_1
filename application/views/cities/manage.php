...
{
	var table_columns = ['','id_city','name'];
	enable_sorting("<?php echo site_url("$controller_name/sorting"); ?>",table_columns, <?php echo $per_page; ?>);
    enable_select_all();
    enable_checkboxes();
    enable_row_selection();
    enable_search('<?php echo site_url("$controller_name/suggest");?>',<?php echo json_encode(lang("common_confirm_search"));?>);
    enable_delete(<?php echo json_encode('Bạn chắc chắn muốn xóa thành phố/ tên nước ?');?>,<?php echo json_encode('Bạn chưa chọn thành phố/ tên nước nào để xóa');?>);
...
<table id="title_bar_new">
	<tr>
		...
		<a href="<?php echo base_url()?>items">
			<div class="newface_back"></div>
		</a> 
		</td>
		<td id="title" style="width: 300px">&nbsp;<?php echo lang('module_'.$controller_name); ?></td>
		<td id="title_search_new"><?php echo form_open("$controller_name/search",array('id'=>'search_form')); ?>
		<input type="text" name='search' id='search'
			placeholder="Nhập tên thành phố/ tên nước ... " /> <img
			src='<?php echo base_url()?>images/spinner_small.gif' alt='spinner'
			id='spinner' />
		</form>
		</td>
	</tr>
...
<table id="contents">
	<tr>
		<td id="commands">
		<div id="new_button"><?php if ($this->Employee->has_module_action_permission($controller_name, 'add_update', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
		<?php echo
		anchor("$controller_name/view/-1/width~380",
				'Tạo mới Thành phố/Tên nước',
				array('class'=>'thickbox none new',
				'title'=>'Tạo mới Thành phố/Tên nước'
		));
		?> <?php } ?> <?php if ($this->Employee->has_module_action_permission($controller_name, 'delete', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
		<?php echo
