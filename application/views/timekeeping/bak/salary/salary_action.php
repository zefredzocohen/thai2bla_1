...
if(count($person_info) > 0 ){
    foreach($person_info AS $key => $values){
        ...
        <?php
        //Tổng tiền phụ cấp khen thưởng nhân viên
                ...
                $.post(url,
                    {
                        ...
                        if(success){
                            /* $('.text_em_salary_basic_'+id).html(em_salary_basic);
                             $('.text_em_wage_level_coverage_'+id).html(em_wage_level_coverage);
                             $('.text_total_x150_'+id).html(total_x150);
                             $('.text_total_x200_'+id).html(total_x200);
                             $('.text_total_x300_'+id).html(total_x300);
                             $('.text_pc_lunch_'+id).html(pc_lunch);
                             $('.text_pc_seniority_'+id).html(pc_seniority);
                             $('.text_pc_position_'+id).html(pc_position);
                             $('.text_pc_project_'+id).html(pc_project);
                             $('.text_pc_computer_'+id).html(pc_computer);
                             $('.text_pc_petrol_phone_'+id).html(pc_petrol_phone);
                             $('.text_pc_other_support_'+id).html(pc_other_support);
                             $('.text_money_custody_'+id).html(money_custody);
                             $('.text_money_advance_'+id).html(money_advance);
                             $('.text_total_real_wages_'+id).html(total_real_wages);
                             $('.text_expected_salary_1_'+id).html(expected_salary_1);
                             $('.text_expected_salary_2_'+id).html(expected_salary_2);
                             $('.text_account_complete_again_'+id).html(account_complete_again);*/
