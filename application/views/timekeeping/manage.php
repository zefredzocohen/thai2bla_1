...
<table id="title_bar_new">
  <tr>
	...
	<?php echo form_open("$controller_name/search",array('id'=>'search_form')); ?>
        <input placeholder="Ngày hoặc mã chấm công cần tìm ..." type="text" name ='search' id='search'/>
...
<table id="contents">
  <tr>
    <td id="commands"><div id="new_button">
        ...
        <?php echo 
					anchor("$controller_name/view/-1/width~$form_width/height~490",
        ...
        <?php //if ($this->Employee->has_module_action_permission($controller_name, 'search', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
            <?php echo anchor("$controller_name/timekeepings/",
                'Bảng chấm công',
                array('class'=>'none new', 'title'=>'Thống kê chấm công'));
            ?>
        <?php //} ?>

        <?php if ($this->Employee->has_module_action_permission($controller_name, 'add_update', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
            <?php echo anchor("$controller_name/payrolls",
                'Bảng tính lương',
                array('class'=>'none new', 'title'=>'Bảng lương'));
            ?>
        <?php } ?>
        <?php if ($this->Employee->has_module_action_permission($controller_name, 'add_update', $this->Employee->get_logged_in_employee_info()->person_id)) {?>
            <?php echo anchor("salaryconfig",
                'Quản lý thuộc tính lương',
                array('class'=>'none new', 'title'=>'Quản lý thuộc tính lương'));
            ?>
            <?php echo anchor("salaryoption",
                'Quản lý cấu hình lương',
                array('class'=>'none new', 'title'=>'Quản lý cấu hình lương'));
            ?>
        <?php } ?>
