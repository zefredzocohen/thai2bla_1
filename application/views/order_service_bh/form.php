...
<?= form_open('costs/save_order_service_bh/'.$var_info1->id,array('id'=>'assets_form'));?>
...
<tr><td>           
        ...
        <?php
        if($var_info1->person_id != 0){
            if($this->Supplier->exists($var_info1->person_id))
                $person_name = $this->Supplier->get_info($var_info1->person_id)->company_name;
            elseif($this->Customer->exists($var_info1->person_id))    
                $person_name = $this->Customer->get_info($var_info1->person_id)->first_name.' '.$this->Customer->get_info($var_info1->person_id)->last_name;
            elseif($this->Employee->exists($var_info1->person_id))    
                $person_name = $this->Employee->get_info($var_info1->person_id)->first_name.' '.$this->Employee->get_info($var_info1->person_id)->last_name;
            ?>
            <tr>
                <td class=td_center ><a href='#' onclick='return deleteCostSupplierRow(this);'>X</a></td>
                <td><?= $person_name ?></td>
                <input type='hidden' name='person_id' id=person_id value='<?= $var_info1->person_id ?>'/>
            </tr>
        <?php
        }?>
    </table><br>
...
<tr><td> 
    ...
    <?php echo form_label(lang('common_ngay_lap').':', 'ngay_lap', array('class'=>'')); ?>
        ...
        <?php echo form_input(array(
            'name'=>'create_date',
            'id'=>'create_date',
            'value'=> $var_info1->create_date ? date('d-m-Y', strtotime($var_info1->create_date)) : ''
        ));?>
        </div>
    </div>
</td><td>
    ...
    <?php echo form_label('Ngày hóa đơn:', 'ngay_lap', array('class'=>'')); ?>
        ...
        <?php echo form_input(array(
            'name'=>'order_date',
            'id'=>'order_date',
            'value'=> $var_info1->order_date ? date('d-m-Y', strtotime($var_info1->order_date)) : ''
        ));?>
        </div>
    </div>
</td></tr>
...
<div class="field_row clearfix">
    ...
    <table class='flower_table'>
        ...
        <tbody>
            ...
            $tax_money = $money_order * $var_info1->tax_percent / 100;
            echo form_input(array(
                name => count_i,
                'class' => count_i,
                type => hidden,
