...
function post_tinhoc_form_submit(response)
{
    if(!response.success)
    {
    ...
    { 
        ...
        if(jQuery.inArray(response.id_tinhoc,get_visible_checkbox_ids()) != -1)
        {
            
            update_row(response.id_tinhoc,'<?php echo site_url("$controller_name/get_row")?>');
        ...
        else //refresh entire table
        {
            ...
            {
                //highlight new row
                highlight_row(response.id_tinhoc);
                set_feedback(response.message,'success_message',false);     
            });
        }
    }
