...
<script type="text/javascript">
    $(document).ready(function () {
        var table_columns = ['', 'code_production_template', '', 'start_date', 'end_date', 'status', ''];
        enable_sorting("<?php echo site_url("$controller_name/sorting_estimate"); ?>", table_columns, <?php echo $per_page; ?>);
        enable_select_all();
        enable_checkboxes();
        enable_row_selection();
        enable_search('<?php echo site_url("$controller_name/suggest_estimate"); ?>',<?php echo json_encode(lang("common_confirm_search")); ?>);               
        enable_delete(<?php echo json_encode('Bạn có chắc chắn muốn xóa những mẫu sản xuất đã chọn ?'); ?>,<?php echo json_encode('Bạn chưa chọn mẫu sản xuất nào để xóa !'); ?>);         
    });
    function post_item_kit_form_submit(response) {
        ...
        <table id="title_bar_new">
            <tr>
                ...
                <td id="title" style="width: 600px">
                    <?php echo '&nbsp; Quản lý mẫu sản xuất: ' . $info_item_kit->name; ?>
                </td>
                <td id="title_search_new" style="width: 200px;">
                    <?php echo form_open("$controller_name/search_estimate/" . $item_kit_id, array('id' => 'search_form')); ?>
                    <input type="text" name ='search' id='search' placeholder="Mã mẫu sản xuất ... "/>
        ...
        <table id="contents" >
            <tr>
                <td id="commands">
                    <div id="new_button">
                        <?php echo anchor("$controller_name/delete_estimate", lang("common_delete"), array(
                                'id' => 'delete',
