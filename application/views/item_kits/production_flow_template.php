    ...
    <div id="content_area">
        <script type="text/javascript">
            $(document).ready(function () {
                var table_columns = ['', 'production_order', 'name_processes', 'time_processes', 'unit_time',''];
                enable_sorting("<?php echo site_url("$controller_name/sorting_production_flow_template"); ?>", table_columns, <?php echo $per_page; ?>);
                enable_select_all();
                enable_checkboxes();
                enable_row_selection();
                enable_search('<?php echo site_url("$controller_name/suggest_production_flow_template"); ?>',<?php echo json_encode(lang("common_confirm_search")); ?>);                
            });
            function post_item_kit_form_submit(response) {
                if (!response.success) {
                    ...
                    if (jQuery.inArray(response.item_id, get_visible_checkbox_ids()) != -1) {
                        update_row(response.item_id, '<?php echo site_url("$controller_name/get_production_flow_template_data_row") ?>');
        ...
        <table id="title_bar_new">
            <tr>
                ...
                <td id="title" style="width: 600px">
                    <?php echo '&nbsp;'.lang('item_kits_manage_processes').': (' . $info_item_kit->name . ' - Mẫu TK: ' . $info_design_template->code_design_template .')'; ?>
                </td>
                <td id="title_search_new" style="width: 200px;">
                    <?php echo form_open("$controller_name/search_production_flow_template/" . $item_kit_id, array('id' => 'search_form')); ?>
                    <input type="text" name ='search' id='search' placeholder="Công đoạn sản xuất ... "/>
        ...
        <table id="contents" >
            <tr>
                <td id="commands">
                    <div id="new_button">
                        <?php echo anchor("$controller_name/view_production_flow_template/-1/width~500", lang($controller_name . '_choose_processes'), array(
                                'class' => 'thickbox none new',
                                'title' => lang($controller_name . '_choose_processes')
                            ));
