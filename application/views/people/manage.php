...
{ 
	...
	enable_sendsms(<?php echo json_encode('Vui lòng chọn khách hàng để gửi SMS')?>,<?php echo json_encode('Bạn chỉ gửi được 1KH/1lần')?>);

//        timer_end_name
        
        
        $('#timer_start_name').datePicker({startDate: '01-01-1950'}).bind(
                'dpClosed',
                function (e, selectedDates) {
                    var d = selectedDates[0];
                    if (d) {
                        d = new Date(d);
                        $('#timer_end_name').dpSetStartDate(d.addDays(1).asString());
                    }
                }
        );
        $('#timer_end_name').datePicker({startDate: '01-01-1950'}).bind(
                        'dpClosed',
                        function (e, selectedDates) {
                            var d = selectedDates[0];
                            if (d) {
                                d = new Date(d);
                                $('#timer_start_name').dpSetStartDate(d.addDays(-1).asString());
                            }
                        }
                );
        
//        $('#money_start_name').keydown(function(event) {
...
//            });
 ...
 $("#money_start_name").keydown(function (e) {
     ...
     $("#money_end_name").keydown(function (e) {
        // Allow: backspace, delete, tab, escape, enter and .
        if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
             // Allow: Ctrl+A, Command+A
            (e.keyCode == 65 && ( e.ctrlKey === true || e.metaKey === true ) ) || 
...
//        menufilter.css({'display':'none'});
    ...
    $("#btn-filter-name").click(function(e){
         e.preventDefault();
         console.log('btn-filter-name submit manage.php');
    ...
    if($){
        ...
        data +="filter_id=1&"
        var timer_start = $("#timer_start_name").val();
        data +="timer_start="+timer_start;
        
        var timer_end = $("#timer_end_name").val();
        data +="&timer_end="+timer_end;
        
        var money_start = $("#money_start_name").val();
        data +="&money_start="+money_start;
        
        var money_end = $("#money_end_name").val();
        data +="&money_end="+money_end;
        
        $.ajax({
            type:"post",
            url: "<?php echo site_url().'/customers/save_session_filter';?>",
            data: data,
            success: function(data){
...
//        $("#filter_value").val(
//                $("#timer_start_name").val()
//                +":"
//                +$("#timer_end_name").val()
//                +":"
//                +$("#money_start_name").val()
//                +":"
//                +$("#money_end_name").val());
//        console.log("timer_end_name");
...
function post_person_form_submit(response){
    if(!response.success){
        ...
        $.get('<?php echo site_url("customers/get_number_birthday");?>',{},function(data){
...
<table id="title_bar_new">
    <tr>
        <td id="title_icon">
            <a href="<?php echo base_url()?><?php $controller_name; ?>"><img src='<?php echo base_url()?>images/menubar/<?php echo $controller_name; ?>.png' alt='title icon' /></a>
        </td>
        <?php if($controller_name == 'customers') {?>
        <td id="title">
            <a href="<?php echo base_url()?><?php echo $controller_name; ?>"><?php echo lang('module_'.$controller_name); ?></a>
        </td>
        <?php }else{?>
        ...
        <td id="title_search_new" >
            <?php echo form_open("$controller_name/search",array('id'=>'search_form','class'=>'customer-search')); ?>
            <?php if($controller_name == 'customers') { 
                $employees = $this->Employee->get_list_employee();
            ?>
                <?php if($this->session->userdata('person_id')==1){ ?>
                <div style="position: absolute; right: 527px; margin-top: 1px;">
                ...
                <?php } ?>
                <div style="position: absolute; right: 330px; margin-top: 1px;">
                ...
                <input type="text" name ='search' id='search' class="customer-search-title" placeholder="Tên hoặc số điện thoại ... " style="margin: 0 !important;" />
                <input type="hidden" name ='filter_value' id='filter_value' class="filter_value" />
                <a id="btn-filter" style="position: absolute;right: 215px; padding: 0px 7px; background: white none repeat scroll 0% 0%;height: 30px !important;margin-top: 0px;font-size: 15px;line-height: 32px;color: rgb(85, 85, 85);font-family: 'OpenSansLight', sans-serif;" >Điều kiện lọc</a>
                <div id="menu-filter" class="menu-togged" style="width: 500px !important;margin-top: 17px; display: none;">
                    <?php?>
                    <table class="mytable filter" cellspacing="0" style="width: 100%; margin: 0px !important;">
                         <thead>
                            <tr>                                            
                                <th style="text-align:center;">lọc theo thời gian</th>
                                <th style="text-align:center;">lọc theo doanh thu Kh</th>
                            </tr>                                      
                        </thead>
                        <tbody>                         
                            <tr>
...
//                                    'value'=>'2016/2/29',
                                ...
                                <td> <?php echo form_input(array(
                                    'name'=>'money_start_name',
                                    'size'=>'30',
                                    'id'=>'money_start_name',
                                    'class'=>'required',
                                    'value'=>'',
                                    'placeholder'=>"Doanh thu tính từ")
                                );?>
                                </td>
                            </tr>
                            <tr>
...
//                                    'value'=>'2016/3/3',
                                ...
                                <td> 
                                    ...
                                    'id'=>'money_end_name',
                                        'class'=>'required',
                                        'value'=>'',
                                    'placeholder'=>"Doanh thu tính đến")
                                );?></td>
                            </tr>
                            <tr>
                                <td colspan="2">
                                    <?php
                                     echo form_open(base_url().'customers/filter/',array('id'=>'filter_form'));
                ...
                <?php } else{?>
                <input type="text" name ='search' id='search' class="customer-search-title" placeholder="Tên hoặc số điện thoại ... "  />
                <a id="btn-filter" style="position: absolute;right: 215px; padding: 0px 7px; background: white none repeat scroll 0% 0%;height: 30px !important;margin-top: 0px;font-size: 15px;line-height: 32px;color: rgb(85, 85, 85);font-family: 'OpenSansLight', sans-serif;"  >Điều kiện lọc</a>
                <div id="menu-filter" class="menu-togged" style="width: 500px !important;margin-top: 17px; display: none;">
                    <?php?>
                    <table class="mytable filter" cellspacing="0" style="width: 100%; margin: 0px !important;">
...
uyen -->
                    ...
                    if ($controller_name =='customers') {
                        ...
                        echo anchor("quotes_contract", 'Mẫu báo giá - hợp đồng', array(
                            'class' => 'quotes_contract none new',
                            'id' => 'quotes_contract',
                            'title' => 'Mẫu báo giá - hợp đồng',
                        ));
                    }
                ?>
            </div>
...
<style>
    ...
    #btn-filter #menu-filter{
            
    }
    #timer_start_name,#timer_end_name {
    width: 80% !important;
}
    #tab_show li a{
        color: #FFF;
