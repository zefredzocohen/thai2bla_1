        ...
        <div style="color:#000;">
            <fieldset id="customer_basic_info" style="margin-bottom: 10px;padding-top: 10px; width: 98% !important">
                ...
                <div id="history_customer" style="width: 700px!important;">
                    ...
                    <p><label>Số ĐTDĐ:</label> <?php echo $customer_info->phone_number; ?> </p>
                    <p><label>Số ĐT cố định:</label> <?php echo $customer_info->phone; ?> </p>
                                ...
                                foreach ($sale_all_invs as $sale_all) {
                                    if (!(($sale_all['suspended'] == 0) && ($sale_all['liability'] == 0) && ($sale_all['materials'] == 1))) {  // th báo giá
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo date('d-m-Y', strtotime($sale_all['sale_time'])); ?></td>
                                            ...
                                            <?php 
                                            foreach ($data_sale as $key => $val1) { 
                                                
                                                  $total_thanhtoan += $val1['pays_amount'];
                                            }
                                            ?>
                                            <td style="">
                                            <?php 
                                                if($total_cost <= ($to+$do)){
                                                  echo 'Tiền mặt: '.number_format($total_cost);
                                              }
                                            ?>
                                                <?php foreach ($data_sale as $key => $val2) { 
                                                ?>
                                            <?php
                                              if($total_cost > ($to+$do)){
                                                 echo $val2['pays_type'] . ': ' . number_format($val2['pays_amount']);
                                              }
                                            ?> 
                                                    <br>
            <?php } ?>
...
<?php echo 'Thống kê lịch sử mua hàng của khách hàng'; ?>
                    ...
                    if ($sale_complete_invs != null) {
                        foreach ($sale_complete_invs as $sale_complete) {
                            ?>
                                            <tr>
                                                <td><a href="<?php echo base_url(); ?>customers/switch_sale/<?php echo $sale_complete['sale_id']; ?>" ><?php echo $sale_complete['sale_id']; ?></a></td>
                            ...
                            <tr class="title_color">
                                ...
                                if ($cost_complete != null) {
                                    foreach ($cost_complete as $cost_completes) { 
                                        $total_cost_money += $cost_completes['money'];
                                        ?>
                                    <tr>
                                        ...
                                        <td style="text-align: right;">
                                            <?php
                                            $t = $this->Sale->get_info($cost_completes['id_sale'])->row()->later_cost_price;
                                             $data_sale_payment1 = $this->Sale->get_payment_sale_by_sale_id($cost_completes['id_sale']);
                                                ...
                                                $cost = $this->Customer->get_customer_money_cost($customer_id,$cost_completes['id_sale']);
                                                foreach ($cost->result() as $cost_sale) {
                                                       $total_cost_id = $this->Cost->get_info($cost_sale->id_cost)->money;
                                                }
                                                
                                            if($cost_completes['form_cost'] == 0){
                                    ...
                                    <?php
                                 //trừ đơn hàng từ trên xuống dưới    
                                }
...
<?php $sum_money = 0; ?>			
                                    ...
                                    <tr class="row_inventory_sale">
                                        <td class="row_sale_id" style="text-align: center;">
                                            <a href="<?php echo base_url(); ?>customers/switch_sale/<?php echo $sale_complete['sale_id']; ?>" ><?php echo $sale_complete['sale_id']; ?></a>
                                        </td>
                                        <td style="text-align: center;"><?php echo date('d-m-Y H:i:s', strtotime($sale_complete['sale_time'])); ?></td>
                                        ...
                                        ?>&nbsp;
                                                ...
                                                <?php
                                                $data_sale_tam = $this->Sale->get_sales_tam($sale_complete['sale_id']);
                                                $data_sale_payment = $this->Sale->get_payment_sale_by_sale_id1($sale_complete['sale_id']);
                                                $data_sale_payment1 = $this->Sale->get_payment_sale_by_sale_id($sale_complete['sale_id']);
                                                $to = 0; //Tiền khách còn nợ của đơn hàng tương ứng
                                                $do = 0; //Tiền chiết khấu cho khách hàng của đơn hàng tương ứng
                                                foreach ($data_sale_payment1 as $val1){
                                                    $to = $to + $val1['payment_amount'];
                                        ...
                                        <td style="text-align: right;"><?= number_format($do); ?>&nbsp;</td>
                                         ...
                                         <td style="text-align: right;"><?= $sale_complete['comment'];?></td>
                                       <?php
                                            $total_cost_sale = 0;
                                            if($cost != NULL){
                                                foreach ($cost->result() as $cost_sale){
                                                    if($sale_complete['sale_id'] == $cost_sale->id_sale){
                                                        $total_cost_sale += $cost_sale->money;
                                                    }
                                                }
                                            }
                                       ?>
                                        <td style="text-align: right;" class="consideration_paid">
                                            <?php 
                                            if($total_cost - $to - $do <=0){
//                                                $data_sale_customer = array(
//                                                    'suspended' => 0
//                                                );
//                                                $this->Sale->update_sale_customer($data_sale_customer,$sale_complete['sale_id']);
                                                echo 0;
                                            }else{
                                               echo number_format($total_cost - $to - $do); 
                                            }
                                            ?>
                                        </td>
                                            <?php
    ...
    if ($num_rows4 > $number_of_items_per_page) {
        ?>
                            ...
                            ?>
                                <tr style="text-align: center">
                                    <td colspan="10"  style="text-align: center !important">Không có giao dịch nào!</td>
                                </tr>
<?php } ?>
                        </table>
