...
function get_affiliates_manage_table_data_rows($employees_jobs,$controller)
{
    ...
    {
        $table_data_rows.="<tr><td colspan='7'><div class='warning_message' style='padding:7px;font-weight:bold;color:#333'>".lang('common_no_affiliates_to_display')."</div></tr></tr>";
    }
...
function get_affiliates_data_row($data,$controller)
{
	...
	$first_name= $CI->Employee->get_info_one_hit2($data->person_id)->first_name;
    /* giang 4/7/2014 */
    $table_data_row='<tr>';
        $table_data_row.="<td width='1%'><input type='checkbox' id='person_$data->jobs_affiliates_id' value='".$data->jobs_affiliates_id."'/></td>";
		$table_data_row.='<td width="15%" style="color:#000;;color:'.$data->jobs_affiliates_color.'""><p style="display:inline-block;margin:0px 0 0 2px;min-height:20px;height:auto;float:left;width:130px;padding:8px 0 5px 0;color:'.$data->jobs_affiliates_color.'">'
        	.anchor("employees/switch_jobs_affiliates/$data->jobs_affiliates_id", $data->jobs_affiliates_name,array( 'title'=>lang($controller_name.'_update'))).
        '</p></td>';        
        $table_data_row.='<td style="color: #000;width: 18%;text-align:left;font-size: 12px">'. $first_name.' </td>';
//        $table_data_row.='<td style="color: #000;width: 18%;text-align:left;font-size: 12px">'. $data->jobs_affiliates_name.' </td>';
...
//            }
//        }
//        $table_data_row.='<td style="color: #000;width: 18%;text-align:left;font-size: 12px">'. $data->first_name.' </td>';
		...
		$table_data_row.='<td style="color: #000;width: 13%;text-align:left;font-size: 12px">'. $data->jobs_city_name.' </td>';
...
//        }
        ...
        $table_data_row.='<td width="13%" style="color:#000;;line-">'.date(get_date_format(),strtotime($data->jobs_affiliates_date)).'</td>';
          /*  if($data->jobs_affiliates_status == 1){
                $table_data_row.='<td width="12%" style="color:#000;;">Hoạt động</td>';
            }else{
                $table_data_row.='<td width="12%" style="color:#000;;">Tạm dừng</td>';
            }*/
    /* giang 4/7/2014 */
