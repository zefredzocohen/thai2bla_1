...
// check parentid (select option)
    ...
    function insert($data) {
        $this->db->insert('categories_item', $data);
    }
    ...
    function update_cat($id, $value) {
        $this->db->where('id_cat', $id);
        $this->db->update('categories_item', $value);
    }
    ...
    function search($search, $cat_service = '', $limit = 20, $offset = 0, $column = 'name', $orderby = 'asc') {
        ...
        if ($this->config->item('speed_up_search_queries')) {
			...
			from (
			         (select *
			         from " . $this->db->dbprefix('categories_item') . "
    ...
    function get_info($id_cat) {
        ...
        if ($query->num_rows() == 1) {
            ...
            $fields = $this->db->list_fields('categories_item');
    ...
    function exists_id_cat($id_cat) {
        ...
        $this->db->where('cat_service', 0);
        $this->db->where('deleted',0);
        return $query = $this->db->get('categories_item')->num_rows();
    }
    //end dung
