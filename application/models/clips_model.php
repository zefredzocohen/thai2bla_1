...
class Clips_model extends CI_Model {
    ...
    public function video_by_id(){
        $this->db->where('id',1);
        return $this->db->get('clip')->result();
    }
    public function video_by_social(){
        $this->db->where('anh',5);
        return $this->db->get('clip')->result();
    }
    ...
    function get_info($id_support) {
        ...
        if ($query->num_rows() == 1) {
            ...
            $fields = $this->db->list_fields('clip');
    ...
    function save(&$item_data, $id_unit = false) {
        if (!$id_unit or ! $this->exists($id_unit)) {
            ...
            $this->db->where('id', $id_unit);
            return $this->db->update('clip', $item_data);
        }
    }
