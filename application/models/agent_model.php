...
class Agent_model extends Person {
...
//    }    
    ...
    // lấy thông tin từ bảng omc_customer //
...
//        Created by San
    ...
    function get_all_by_employee_id($employee_id, $limit = 10000, $offset = 0, $col = 'lifetek_customers.person_id', $order = 'desc') {
        ...
        if ($employee_id == 1) {
            $data = $this->db->query("SELECT *, $customers.company_name
                ...
                STRAIGHT_JOIN " . $customers . " ON 									                       
                " . $people . ".person_id = " . $customers . ".person_id
                WHERE ".$customers.".deleted =0 AND ".$customers.".status=1 ORDER BY " . $col . " " . $order . " 
                LIMIT  " . $offset . "," . $limit);
        } else {
            $data = $this->db->query("SELECT *,$customers.company_name
                FROM " . $people . "
                STRAIGHT_JOIN " . $customers . " ON 									                       
                " . $people . ".person_id = " . $customers . ".person_id
                WHERE ".$customers.".deleted =0 AND employee_id=" . $employee_id . " AND ".$customers.".status=1 ORDER BY " . $col . " " . $order . " 
                LIMIT  " . $offset . "," . $limit);
        }
        return $data;
    }
    ...
    function search_by_employee_id($employee_id, $search, $employees, $type_customer, $limit = 20, $offset = 0, $column = 'last_name', $orderby = 'asc') {
        if ($type_customer)
            $type_customer_id = " and type_customer = $type_customer ";
        else
            $type_customer_id = '';
        ...
        if ($this->config->item('speed_up_search_queries')) {
            ...
            if ($employee_id == 1) {
                if ($employees)
                    $employee = " and employee_id = $employees";
                else
                    $employee = "";
                $this->db->from('customers');
                $this->db->join('people', 'customers.person_id=people.person_id');
                ...
                phone_number LIKE '%" . $this->db->escape_like_str($search) . "%' or 
                account_number LIKE '%" . $this->db->escape_like_str($search) . "%' or 
                company_name LIKE '%" . $this->db->escape_like_str($search) . "%' ) $employee $type_customer_id and deleted=0");
                $this->db->order_by($column, $orderby);
                $this->db->limit($limit);
                $this->db->offset($offset);
                return $this->db->get();
    ...
    function search_count_all_by_employee_id($employee_id, $search, $employees = '', $type_customer = '', $limit = 10000) {
        if ($type_customer)
            $type_customer_id = "and type_customer = $type_customer ";
        else
            $type_customer_id = '';
        ...
        if ($this->config->item('speed_up_search_queries')) {
            ...
            if ($employee_id == 1) {
                ...
                account_number LIKE '%" . $this->db->escape_like_str($search) . "%' or 
                company_name LIKE '%" . $this->db->escape_like_str($search) . "%' or 
                CONCAT(`first_name`,' ',`last_name`) LIKE '%" . $this->db->escape_like_str($search) . "%') $employee $type_customer_id and deleted=0");
                $this->db->order_by("last_name", "asc");
                $this->db->limit($limit);
                $result = $this->db->get();
                return $result->num_rows();
...
//        end Loi
    ...
    //check trùng tên 
    public function check_code_customer($code_customer, $person_id='') {
       if($person_id){
           $this->db->where('person_id !=',$person_id);
       }
       $this->db->where('code_customer',$code_customer);
