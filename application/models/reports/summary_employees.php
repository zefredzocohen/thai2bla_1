...
class Summary_employees extends Report
{
	...
	{
		$this->db->select('CONCAT_WS(" ",first_name, last_name) as employee, sum(subtotal) as subtotal, 
	...
	{
		$this->db->select('sum(subtotal) as subtotal, sum(taxes * subtotal/100) as tax, sum(total) as total, sum(profit) as profit');
		$this->db->from('sales_items_temp');
		$this->db->join('employees', 'employees.person_id = sales_items_temp.employee_id');
		$this->db->join('items', 'sales_items_temp.item_id = items.item_id');
		$this->db->join('people', 'employees.person_id = people.person_id');
