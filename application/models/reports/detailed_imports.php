...
class Detailed_imports extends Report {
    ...
    public function getDataColumns() {
        return array('summary' => array(
                ...
                array('data' => 'Ngoại tệ', 'align' => 'center'),
    ...
    public function getData() {
        $this->db->select('supplier_id ,receiving_id, receiving_date, sum(quantity_purchased) as items_purchased, supplier_id,CONCAT_WS(employee.first_name," ",employee.last_name) as employee_name, supplier.first_name as supplier_name, sum(total) as total, sum(profit) as profit, payment_type, comment, status_currency, currency_id', false);
        ...
        foreach ($data['summary'] as $key => $value) {
            $this->db->select('items.name as name,units.name as units_name, categories_item.name as cat_name,item_unit_price, quantity_purchased, serialnumber,total, discount_percent, receivings_items_temp.taxes as taxes');
            $this->db->from('receivings_items_temp');
