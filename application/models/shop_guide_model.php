...
class Shop_guide_model extends CI_Model {
    ...
    function insert($data) {
        $this->db->insert('shop_guide', $data);
    }
    ...
    function get_info($id_support) {
        ...
        if ($query->num_rows() == 1) {
            ...
            $fields = $this->db->list_fields('shop_guide');
    ...
    function save(&$item_data, $id_unit = false) {
        if (!$id_unit or ! $this->exists($id_unit)) {
            ...
            $this->db->where('id', $id_unit);
            return $this->db->update('shop_guide', $item_data);
        }
    }
