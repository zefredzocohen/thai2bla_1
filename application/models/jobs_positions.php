<?php
class Jobs_positions extends Employee
{
...
//	}
    ...
    function get_info($id)
	{
		...
		else
		{
			...
			$fields = $this->db->list_fields('jobs_positions');
...
//    }
    ...
    function save(&$item_data,$jobs_positions_id = -1)
    {
		
        if($jobs_positions_id == -1)
        {
        ...
        $this->db->where('jobs_positions_id', $jobs_positions_id);
    ...
    */
     ...
     * */
    function checkRegionsName($jobs_positions_name,$jobs_positions_id)
    {
        if(empty($jobs_positions_id) OR $jobs_positions_id == -1){
            $this->db->where("jobs_positions_name LIKE '".$this->db->escape_like_str($jobs_positions_name)."'");
        }else{
            $this->db->where("jobs_positions_name LIKE '".$this->db->escape_like_str($jobs_positions_name)."' AND jobs_positions_id != ".$jobs_positions_id);
        }
