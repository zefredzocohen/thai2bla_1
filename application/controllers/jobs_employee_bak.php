    ...
    */
    ...
    public function save_one($jobs_id = '')
    {
        ...
        $config = array(
            ...
            'allowed_types' => 'doc|docx|pdf|xls|xlsx|rar|zip',
        );
        $this->load->library('upload', $config);
        $this->upload->do_upload('employees_jobs_file');
        // $file_data_contract_vitae=$this->upload->data(); 
        if (!$this->upload->do_upload('employees_jobs_file')) {
            $error = array(
                'errors' => $this->upload->display_errors(),
            );
        } else {
            $file_data_file_emp = $this->upload->data();
        }
        if ($jobs_id != -1) {
            $data = $this->Jobs_employees->get_info_one_file_emp($jobs_id);
            if (empty($file_data_file_emp)) {
                $file_data_file_emp['file_name'] = $data->employees_jobs_file;
            }
        }
        $this->check_action_permission('add_update');
        if ( !$this->upload->do_upload('jobs_file_url')){
            $jobs_data = array(
                ...
                'employees_jobs_file'=>$file_data_file_emp['file_name'],
                'project_status'=>'0',
                'jobs_approve'=>'0',
        ...
        if($this->Jobs_employees->checkName($this->input->post('jobs_name'),$jobs_id) > 0){
           echo "Tên này: (".$this->input->post('jobs_name').') đã được sử dụng .';
            ...
            {
                header('Location:'.base_url().'jobs_employee');
            }
            else{
