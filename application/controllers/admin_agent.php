...
class Admin_agent extends Person_controller {

    function __construct() {
//        date_default_timezone_set('Asia/Ho_Chi_Minh');
        parent::__construct('admin_agent');
        $this->load->model('Agent_model');
        $this->load->model('Contractcustomers');
    ...
    function index() {
        ...
        //end phan don hang
        //phan lam
        $data['customer'] = $this->Agent_model->findBirthDate();
        //lay danh sach khach hang sap het hop dong
        $day = date('d-m-Y', time());
        $day_int = strtotime($day);
        $contracAgent_model = $this->Contractcustomers->get_contraccustomer_expried($this->session->userdata('person_id'));
        $list_id = array();
        foreach ($contracAgent_modelas as $v) {
        ...
        //phan lam san pham
        $items = array();
        $allitems = $this->Agent_model->findAllItem();
        ...
        $config['base_url'] = site_url('admin_agent/sorting');
//        $config['total_rows'] = $this->Customer->count_all();
        $config['total_rows'] = $this->Agent_model->count_all_by_employee_id($this->session->userdata('person_id'));
        ...
        $data['manage_table'] = get_agent_manage_table($this->Agent_model->get_all_by_employee_id($this->session->userdata('person_id'), $data['per_page']), $this);
        $data['register_date'] = $this->Agent_model->finddateregister();
        //BEGIN SMS
        $max_id_sms = $this->Agent_model->get_table_number_sms();
        $data['number_sms'] = $this->Agent_model->get_info_id_max_of_table_number_sms($max_id_sms['id']);
        //END SMS
        $this->load->view('people/manage_agent', $data);
    }

    //check trùng tên
    function checkname($id) {
        $first_name = $this->input->post('first_name');
        $d['first_name'] = $this->Agent_model->getname($id);
    ...
    function sorting() {
        ...
        $type_Agent_model = $this->input->post('cat');
...
//            $table_data = $this->Customer->search($search,$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'last_name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
            $config['total_rows'] = $this->Agent_model->search_count_all_by_employee_id($this->session->userdata('person_id'), $search, $employee, $type_customer);
            $table_data = $this->Agent_model->search_by_employee_id($this->session->userdata('person_id'), $search, $employee, $type_customer, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'lifetek_people.person_id', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'desc');
        } else {
            //$config['total_rows'] = $this->Customer->count_all();
            ...
            $config['total_rows'] = $this->Agent_model->count_all_by_employee_id($this->session->userdata('person_id'));
            $table_data = $this->Agent_model->get_all_by_employee_id($this->session->userdata('person_id'), $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'lifetek_people.person_id', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'desc');
        }
        $config['base_url'] = site_url('admin_agent/sorting');
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['manage_table'] = get_agent_manage_table_data_rows($table_data, $this);
        echo json_encode(array('manage_table' => $data['manage_table'], 'pagination' => $data['pagination']));
    }

    /*
      Returns Agent_modeltable data rows. This will be called with AJAX.
     */
...
//            $search_data=$this->Customer->search($search,$type_customer,$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'last_name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
        $search_data = $this->Agent_model->search_by_employee_id($this->session->userdata('person_id'), $search, $employee, $type_customer, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'lifetek_customers.person_id', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'desc');
        $config['base_url'] = site_url("admin_agent/search");
//            $config['total_rows'] = $this->Customer->search_count_all($search,$type_customer);
        $config['total_rows'] = $this->Agent_model->search_count_all_by_employee_id($this->session->userdata('person_id'), $search, $employee, $type_customer);
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['manage_table'] = get_agent_manage_table_data_rows($search_data, $this);
        echo json_encode(array('manage_table' => $data['manage_table'], 'pagination' => $data['pagination']));
    }
    ...
    function suggest() {
        $suggestions = $this->Agent_model->get_search_suggestions_by_employee_id($this->session->userdata('person_id'), $this->input->get('term'), 100);
        echo json_encode($suggestions);
    }

    /*
      Loads the Agent_modeledit form
     */

    function view($customer_id = -1) {
        $this->check_action_permission('add_update');
        $data['person_info'] = $this->Agent_model->get_info_customer($customer_id);
        $data['option'] = $this->Agent_model->get_option1();
        $data['option2'] = $this->Agent_model->get_option2();
        $this->load->view("admin_agent/form", $data);
    }

    function account_number_exists() {
        if ($this->Agent_model->account_number_exists($this->input->post('account_number')))
            echo 'false';
        else
            echo 'true';
    }
    ...
    function finddate() {
        ...
        if ($start_date_get[0] == $end_date_get[0]) {
            if ($start_mon == $end_mon) {
                $findper = $this->Agent_model->findperinmonth($start_mon, $start_day, $end_mon, $end_day);
            }
        };

        $data['manage_table'] = get_people_manage_table($findper, $this);
        $this->load->view('admin_agent/finddate', $data);
        //end phan lam
    }
    ...
    function save($customer_id = -1) {
        ...
        $person_info = $this->Agent_model->get_info_customer($customer_id);
        $this->load->model('Agent_model');
        ...
        if ($customer_id != -1) {
            $customer_data = array(
                ...
                'agent' => $this->input->post('agent_input') ? $id : $this->sale_lib->get_agent(),
                'status' => 1,
                'status_register' => $this->input->post('status_register') ? 1 : 0,
        ...
        if ($this->Agent_model->save($person_data, $customer_data, $customer_id)) {
            ...
            //New customer
            if ($customer_id != -1) {
                echo json_encode(array('success' => true, 'message' => 'Cập nhật đại lý thành công' . ' ' .
                    $person_data['first_name'] . ' ' . $person_data['last_name'], 'person_id' => $customer_id));
            } else { //previous customer
                echo json_encode(array('success' => true, 'message' => 'Thêm mới thành công đại lý' . ' ' .
    ...
    public function check_name_code($person_id) {
        $code_customer = $this->input->post('code_customer');
        if ($person_id) {
            $check = $this->Agent_model->check_code_customer($code_customer, $person_id);
        } else {
            $check = $this->Agent_model->check_code_customer($code_customer);
        }
        if ($check > 0) {
            echo json_encode(false);
        } else {
    ...
    function delete() {
        ...
        if ($this->Agent_model->delete_list($customers_to_delete)) {
            echo json_encode(array('success' => true, 'message' => 'Xóa thành công đại lý' . ' ' .
    ...
    function excel_import() {
        $this->check_action_permission('add_update');
        $this->load->view("admin_agent/excel_import", null);
    }
        ...
        if ($_FILES['file_path']['error'] != UPLOAD_ERR_OK) {
            ...
            if ($objPHPExcel = PHPExcel_IOFactory::load($_FILES['file_path']['tmp_name'])) {
                ...
                for ($row = 3; $row <= $highestRow; ++$row) {
                    ...
                    if (!$this->Agent_model->save($person_data, $customer_data)) {
    ...
    function excel_import_update() {
        $this->check_action_permission('add_update');
        $this->load->view("admin_agent/excel_import_update", null);
    }
        ...
        if ($_FILES['file_path']['error'] != UPLOAD_ERR_OK) {
            ...
            if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE) {
                ...
                while (($data = fgetcsv($handle)) !== FALSE) {
                    ...
                    if ($this->Agent_model->exists($data[14])) {
                        $this->Agent_model->save($person_data, $customer_data, $data[14]);
                    } else if (!$this->Agent_model->save($person_data, $customer_data)) {
                        echo json_encode(array('success' => false, 'message' => lang('customers_duplicate_account_id')));
                        return;
                    }
                }
    ...
    function updatebirthdate() {
        ...
        $this->Agent_model->update_birth_date($data);
    }
    ...
    function updatecompany() {
        ...
        $this->Agent_model->update_company($data);
    }

    function shopping($id_customer) {
        if ($this->Agent_model->exists($id_customer)) {
    ...
    function detail_customer_sale($id_customer) {
        ...
        $data['sms_history'] = $this->Agent_model->get_history_sms($id_customer, $page_limit, $resultsPerPage);
        $data['customer_info'] = $this->Agent_model->get_info($id_customer);
        ...
        $data['mail_history'] = $this->Agent_model->find_mail_history_customer2($id_customer, $page_limit, $resultsPerPage);
        ...
        $this->load->view('admin_agent/detail_customer_sale', $data);
    }
...
//hung dq 28-01
    ...
    function load_more($id_customer) {
        ...
        $this->load->view('admin_agent/tab_ban_hang', $data);
    }
    ...
    function load_more2($id_customer) {
        ...
        $this->load->view('admin_agent/tab_thu_chi', $data);
    }
    ...
    function load_more4($id_customer) {
        ...
        $this->load->view('admin_agent/tab_cong_no', $data);
    }
...
//tab bao gia 5
    function load_more5($id_customer) {
        ...
        $this->load->view('admin_agent/tab_bao_gia', $data);
    }
    ...
    function load_more6($id_customer) {
        ...
        $data['num_rows'] = $this->Agent_model->get_row6($id_customer, $page_limit, $resultsPerPage);
        $data['mail_history'] = $this->Agent_model->find_mail_history_customer2($id_customer, $page_limit, $resultsPerPage);
        $data['resultsPerPage'] = $resultsPerPage;
        $data['paged'] = $paged;

        $this->load->view('admin_agent/tab_gui_mail', $data);
    }
    ...
    function load_more_history_sms($id_customer) {
        ...
        $data['num_rows'] = $this->Agent_model->get_row_history_sms($id_customer, $page_limit, $resultsPerPage);
        $data['sms_history'] = $this->Agent_model->get_history_sms($id_customer, $page_limit, $resultsPerPage);
        $data['resultsPerPage'] = $resultsPerPage;
        $data['paged'] = $paged;

        $this->load->view('admin_agent/tab_history_sms', $data);
    }
        ...
        foreach ($data['liabilities'] as $key => $value) {
            ...
            if ($discount_money <= 0) {
                ...
                if ($money_cus >= 0) {
                    ...
                    $cost_comment = "Thu tiền thanh toán nợ của khách hàng " . $this->Customer->get_info($customer_id)->first_name . ' ' . $this->Agent_model->get_info($customer_id)->last_name;
                    $cost_data = array(
                    ...
                    $this->Sale->insert_payment_sale($data_payment_sale);
                    $cost_comment = "Thu tiền thanh toán nợ của khách hàng " . $this->Customer->get_info($customer_id)->first_name . ' ' . $this->Agent_model->get_info($customer_id)->last_name . ' hình thức trả góp';
                    $cost_data = array(
                        'id_customer' => $customer_id,
                        'name' => 4,
                        'tien_thu' => $value['val'] - $money_cus,
                        'tien_chi' => 0,
                        'date' => date('Y-m-d H:i:s'),
                        'cost_date_ct' => date('Y-m-d'),
                        'comment' => $cost_comment,
...
//                    $data['sale_payment_paid_no'][$value['key']] = $value['val'];
    ...
    function manage_mail() {
        ...
        $config['total_rows'] = $this->Agent_model->count_all_mail();
        $config['per_page'] = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        ...
        $data['manage_table'] = get_mail_manage_table($this->Agent_model->get_all_mail($data['per_page']), $this);
        //$data['list_mails'] = $this->Customer->get_all_mail($data['per_page'])->result();
        //print_r($data['list_mails']);die;
        $this->load->view("admin_agent/manage_mail", $data);
    }
    ...
    function sorting_mail() {
        ...
        $config['total_rows'] = $this->Agent_model->count_all_mail();
        $table_data = $this->Agent_model->get_all_mail($per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'mail_title', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'asc');

        $config['base_url'] = site_url('admin_agent/sorting_mail');
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
    ...
    function create_mail() {
        ...
        $this->load->view("admin_agent/create_mail", $data);
    }
    ...
    function view_mail($mail_id = -1) {
        ...
        $data['mail_info'] = $this->Agent_model->get_info_mail($mail_id);
        $this->load->helper('ckeditor');
    ...
    function save_mail($mail_id = -1) {
        ...
        if ($this->Agent_model->save_mail($mail_data, $mail_id)) {
            if ($mail_id == -1) {
    ...
    function delete_mail() {
        ...
        foreach ($list_mail_template as $key => $value) {
            $info_mail = $this->Agent_model->get_info_mail($value);
        ...
        if ($check) {
            if ($this->Agent_model->delete_mail_list($mails_to_delete)) {
    ...
    function send_mail() {
        ...
        $config['global_xss_filtering'] = FALSE;
        $this->form_validation->set_rules('inhoud', 'inhoud', 'xss|clean');
        $data['list_mail'] = $this->Agent_model->get_all_mail();
        $data['list_customer'] = $this->Agent_model->get_all();
        $list_mail = array('' => lang('items_none'));
        foreach ($this->Agent_model->get_all_mail()->result_array() as $lm) {
        ...
        $this->load->helper('ckeditor');
        $this->load->view("admin_agent/send_mail", $data);
    }
    ...
    function do_send_mail() {
        ...
        $mail_info = $this->Agent_model->get_info_mail($mail_id);
        ...
        if ($check == 1) {
            if (isset($_SESSION['mail']) && $_SESSION['mail'] != NULL) {
                foreach ($_SESSION['mail'] as $mail) {
                    ...
                    if ($mail['email'] != "") {
                        ...
                        $info_contracAgent_model = $this->Contractcustomers->get_info_contraccustomer_by_customer($mail['person_id']);
                        ...
                        if ($this->email->send()) {
                            ...
                            $this->Agent_model->add_mail_history($data_history);
            ...
            foreach ($customer_ids as $item) {
                $info_cus = $this->Agent_model->get_info($item);
                $info_contracAgent_model = $this->Contractcustomers->get_info_contraccustomer_by_customer($info_cus->person_id);
                if ($info_cus->email != "") {
                    $config = Array(
                        'protocol' => 'smtp',
                        'smtp_host' => 'ssl://smtp.googlemail.com',
    ...
    public function auto_send_mail() {
        ...
        //get all Agent_modelhas birthday on current day
        $list_Agent_model = $this->Agent_model->get_multiple_birthday_info($birthday)->result();
        //get list_email of Agent_modelfrom multi ids
        $list_customer_email = array();
        foreach ($list_Agent_modelas as $l_c) {
            if ($l_c->email != '')
    ...
    function get_row_mail() {
        ...
        $data_row = get_mail_data_row($this->Agent_model->get_info_mail($mail_id), $this);
        echo $data_row;
    }
    ...
    function save_list_send_mail($item_ids) {
        ...
        foreach ($item_ids as $item) {
            $info_cus = $this->Agent_model->get_info_person_by_id($item);
    ...
    function view_mail_history($id) {
        $data['mail_history'] = $this->Agent_model->get_info_mail_history($id);
        $this->load->view("admin_agent/form_mail_history", $data);
    }

    function remove_mail_history($id) {
        $id = $_POST['id'];
        $mail_history = $this->Agent_model->get_info_mail_history($id);
        if ($mail_history['file'] != "" && $mail_history['status'] == 0) {
            unlink(APPPATH . "/../excel_materials/" . $mail_history['file']);
        }
        $this->Agent_model->delete_mail_history($id);
    }

    function save_session_type_customer() {
        $type_Agent_model = $_POST['type_customer'];
        if ($type_customer) {
    ...
    function get_number_birthday() {
        ...
        $Agent_model = $this->Agent_model->findBirthDate();
        foreach ($Agent_modelas as $cus) {
            $cus_mail_auto = $this->Agent_model->get_customer_mail_auto($cus['person_id']);
     ...
     */
    function manage_sms() {
        $config['total_rows'] = $this->Agent_model->count_all_sms();
        $config['per_page'] = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['base_url'] = site_url('admin_agent/sorting_sms');
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['controller_name'] = strtolower(get_class());
        $data['form_width'] = $this->get_form_width();
        $data['per_page'] = $config['per_page'];
        $data['manage_table'] = get_sms_manage_table($this->Agent_model->get_all_sms($data['per_page']), $this);
        $this->load->view("admin_agent/manage_sms", $data);
    }

    function sorting_sms() {
        $per_page = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['total_rows'] = $this->Agent_model->count_all_sms();
        $table_data = $this->Agent_model->get_all_sms($per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'id', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'DESC');
    ...
    function view_sms($id = -1) {
        $data['info_sms'] = $this->Agent_model->get_info_sms($id);
        $this->load->view("admin_agent/form_sms", $data);
    }
    ...
    function save_sms($id = -1) {
        ...
        if ($this->Agent_model->save_sms($sms_data, $id)) {
            if ($id == -1) {
    ...
    function get_row_sms() {
        ...
        $data_row = get_sms_data_row($this->Agent_model->get_info_sms($id), $this);
        echo $data_row;
    }
    ...
    function delete_sms() {
        ...
        if ($this->Agent_model->delete_sms_list($sms_to_delete)) {
    ...
    function send_sms() {
        ...
        $data['list_sms'] = $this->Agent_model->get_all_sms();
        $this->load->view("admin_agent/send_sms", $data);
    }

    function do_send_sms() {
        $customer_ids = $this->input->post('customer_ids');
        $sms_id = $this->input->post('sms_id');
        $info_sms = $this->Agent_model->get_info_sms($sms_id);
        $message = $info_sms->message;
        $max_id_table_number_sms = $this->Agent_model->get_table_number_sms();
        $info_max_id = $this->Agent_model->get_info_id_max_of_table_number_sms($max_id_table_number_sms['id']);
        if ($info_max_id['quantity_sms'] > 0) {
            foreach ($customer_ids as $id_cus) {
                $info_cus = $this->Agent_model->get_info($id_cus);
                ...
                if ($result) {
                    ...
                    $this->Agent_model->save_message($data_insert);
                    if ($result > 0) {
                        $data_update_table_number_sms = array(
                            'quantity_sms' => ($info_max_id['quantity_sms'] - $info_sms->number_message),
                        );
                        $this->Agent_model->update_number_sms($max_id_table_number_sms['id'], $data_update_table_number_sms);
                    }
    ...
    function search_sms() {
        ...
        $search = $this->input->post('search');
        $per_page = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $search_data = $this->Agent_model->search_sms($search, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'id', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'desc');
        $config['base_url'] = site_url("customers/search_sms");
        $config['total_rows'] = $this->Agent_model->search_count_sms($search);
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
    ...
    function suggest_sms() {
        $suggestions = $this->Agent_model->get_search_suggestions_sms($this->input->get('term'), 100);
        echo json_encode($suggestions);
    }

    function get_number_sms() {
        $max_id_sms = $this->Agent_model->get_table_number_sms();
        $sms = $this->Agent_model->get_info_id_max_of_table_number_sms($max_id_sms['id']);
...
//    By Loi
    function phone_number_exists() {
        ...
        $phone_number = $this->input->post("phone_number");
        echo ($this->Agent_model->phone_number_exists($phone_number, $person_id) ? "false" : "true");
    }
...
//    end Loi
    ...
    function statistical($customer_id = -1) {
        $data['person_info'] = $this->Agent_model->get_info_customer($customer_id);
        $this->load->view('admin_agent/statistical', $data);
    }
